...
foreach ($edited as $edit) {
  ...
  $value = $edit[1];
  if ($field === 'img_src') {
    move_uploaded_file($value, "../../../../assets/images/admin.png");
    $queryValues[] = "img_src = '/hagurash/assets/images/admin.png'";
  } else {
    $queryValues[] = "$field = '$value'";
  }
}
array_walk($edited, function ($edit) {
  if ($edit[0] === 'name')
    $_SESSION['admin_name'] = $edit[1];
  if ($edit[0] === 'img_src')
    $_SESSION['admin_img_src'] = "../../../assets/images/admin.png";
});
...
$result = [
  ...
  "result" => [
    "name" => $name,
    "email" => $email,
    "imgSrc" => $imgUploaded ? "/hagurash/assets/images/admin.png" : ""
  ]
];
